...
public class SelectionState : IDrawingState
{
    ...
    public void MouseUp(Point location)
    {
        ...
        if (_isDraggingOrangeDot && _selectedShape != null)
        {
            ...
            if (_totalDeltaX != 0 || _totalDeltaY != 0)
            {
                ...
                _presenter.ExecuteCommand(new MoveTextCommand(_model, _selectedShape,
                    oldTextX, oldTextY,
                    _selectedShape.TextPositionX, _selectedShape.TextPositionY));
            }
        }
    ...
    private void EditShapeText()
    {
        ...
        // 使用view的方法來顯示對話框
        // 修改這行，使用完整的命名空間路徑
        var newText = (_presenter.View as global::MyDrawing.MyDrawing)?.ShowTextEditDialog(oldText);

        // 如果文字有變更
        if (newText != null && newText != oldText)
        {
            // 執行文字修改命令
            _presenter.ExecuteCommand(new ModifyTextCommand(_model, (Shape)_selectedShape, oldText, newText));
